    ...
    *,
    ...
    Args:
        ...
        field_names (Optional[List[str]]): A list of field names that correspond
            to the data in this class. This list must contain all the fields
            that are used to initialize the class. This argument is optional if
            ``cls`` is a dataclass, in which case the fields will be taken from
            ``dataclasses.fields()``.
    ...
    if not dataclasses.is_dataclass(cls):
        assert field_names is not None, (
            "field_names must be specified with a list of all"
            "attribute names if cls is not a dataclass"
        )
    else:
        dataclass_fields = [f.name for f in dataclasses.fields(cls)]
        if field_names is not None:
            assert set(field_names) == set(dataclass_fields), (
                "field_names does not include all dataclass fields:\n"
                f"field_names: {field_names}\n"
                f"dataclass fields: {dataclass_fields}"
            )
        else:
